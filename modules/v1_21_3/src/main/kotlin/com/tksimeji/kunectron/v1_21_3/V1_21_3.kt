package com.tksimeji.kunectron.v1_21_3
...
object V1_21_3: Adapter {
    override val versions: Array<String> = arrayOf("1.21.3", "1.21.4")
    ...
    override fun sendAdvancementGranted(
        ...
        val packet = ClientboundUpdateAdvancementsPacket(false, setOf(holder), emptySet(), mapOf(resourceLocation to progress))
        (player as CraftPlayer).handle.connection.send(packet)
    }

    override fun sendAdvancementCleanup(player: Player, key: Key) {
        val resourceLocation = ResourceLocation.fromNamespaceAndPath(key.namespace(), key.value())
        val packet = ClientboundUpdateAdvancementsPacket(false, emptySet(), setOf(resourceLocation), emptyMap())
        (player as CraftPlayer).handle.connection.send(packet)
    }
    ...
    override fun sendOpenAnvilScreen(player: Player, title: Component): AnvilInventory {
        ...
        val levelAccess = ContainerLevelAccess.create(nmsPlayer.serverLevel(), BlockPos.of(BlockPos.asLong(0, 0, 0)))
